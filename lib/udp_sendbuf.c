...
void sendbuf_flush(udp_send_buffer * sb) {
	...
	for (i = 0; i < sb->connections_count; i++) {
		connection* c =  &sb->connections[i];
...
udp_send_buffer * sendbuf_new(int count, address* addresses) {
	...
	for (i = 0; i < count; i++) {
		...
		c->addr.sin_addr.s_addr = inet_addr(a->address_string);
	    c->addr.sin_family = AF_INET;
	    c->addr.sin_port = htons((uint16_t)a->port);
